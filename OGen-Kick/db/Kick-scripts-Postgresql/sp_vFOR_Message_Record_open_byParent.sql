CREATE OR REPLACE FUNCTION "sp_vFOR_Message_Record_open_byParent"(
...
$BODY$
	...
	BEGIN
		FOR _Output IN
			...
			INNER JOIN "fnc_vFOR_Message_Record_open_byParent"(
				"IDMessage__parent_search_"
			) t2 ON (
				(t2."IDMessage" = t1."IDMessage")
			)

			-- change where condition in: "fnc_vFOR_Message_Record_open_byParent"
			-- NOT HERE!
