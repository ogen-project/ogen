...
%>CREATE PROCEDURE [dbo].[sp0_<%=_aux_table.Name%>_getObject]<%
	...
	@<%=_aux_field.Name%>_ <%=_aux_field.DBs[_aux_dbservertype].DBType_inDB_name%><%=(_aux_field.isText) ? " (" + _aux_field.Size + ")" : ""%><%=(_aux_field.Numeric_Scale > 0) ? " (" + _aux_field.Numeric_Precision + ", " + _aux_field.Numeric_Scale + ")" : ""%> OUT<%=(f != _aux_table.Fields.Count - 1) ? ", " : ""%><%
	}%>
AS
	DECLARE @Exists Bit
	SET @Exists = 0
