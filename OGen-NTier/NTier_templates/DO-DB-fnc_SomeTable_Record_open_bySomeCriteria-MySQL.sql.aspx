...
#endregion
//-----------------------------------------------------------------------------------------
%>CREATE FUNCTION `fnc_<%=_aux_table.Name%>_Record_open_<%=_aux_search.Name%>`(<%
	...
	`<%=_aux_field_name%>_search_` <%=_aux_field.DBs[_aux_dbservertype].DBType_inDB_name%><%=(_aux_field.isText) ? "(" + _aux_field.Size + ")" : ""%><%=(f != _aux_search.SearchParameters.Count - 1) ? ", " : ""%><%
	}%>
)
	RETURNS TABLE
	NOT DETERMINISTIC
...
for (int f = 0; f < _aux_search.SearchParameters.Count; f++) {
	...
	if (_aux_field.isNullable && !_aux_table.isVirtualTable) {%>
		((
			...
			AND
			(`<%=_aux_field.Name%>` <%=(_aux_field.isText) ? "LIKE '%' +" : "="%> `<%=_aux_field_name%>_search_`<%=(_aux_field.isText) ? " + '%' COLLATE " + _aux_field.DBs[_aux_dbservertype].DBCollationName : ""%>)
